...
import { Topic, Grade, UserProgress } from '@/types';
...
const TopicPage = ({ userProgress }: TopicPageProps) => {
  ...
  useEffect(() => {
    ...
    for (const grade of curriculumData.grades) {
      const foundTopic = grade.topics.find(t => t.id === topicId);
      if (foundTopic) {
        setTopic(foundTopic);
        setGrade(grade);
        break;
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden mb-8">
        ...
        <ul className="divide-y divide-gray-200">
          {topic.lessons.map((lesson, index) => {
            ...
            const isLocked = index > 0 && !userProgress.completedLessons.includes(topic.lessons[index - 1].id);
                ...
                <div className="flex items-start">
                  ...
                  <div className="flex-1">
                    ...
                    {isLocked ? (
                      <button 
                        disabled
                        className="btn bg-gray-200 text-gray-500 cursor-not-allowed"
                      >
                      ...
                      <Link 
                        to={`/lesson/${lesson.id}`}
                        className={`btn ${isCompleted ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'btn-primary'}`}
                      >
