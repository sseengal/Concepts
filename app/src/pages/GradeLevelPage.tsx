...
const GradeLevelPage = ({ userProgress }: GradeLevelPageProps) => {
  ...
  useEffect(() => {
    ...
    const foundGrade = curriculumData.grades.find(g => g.id === gradeId);
    setGrade(foundGrade || null);
    setLoading(false);
  }, [gradeId]);

  // Calculate topic completion percentage
  const getTopicCompletion = (topic: Topic) => {
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">
          Explore {grade.topics.length} topics with {grade.topics.reduce((acc, topic) => acc + topic.lessons.length, 0)} interactive lessons
        </p>
      </div>
              ...
              <div className="mb-4">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2.5">
                  <div 
                    ...
                    style={{ width: `${getTopicCompletion(topic)}%` }}
                  ></div>
                </div>
              </div>
